...
import { replaceInvalidFileNameChars } from '@common/utils/aiFileName'
...
export const createDownloadInfo = (musicInfo: LX.Music.MusicInfoOnline, type: LX.Quality, fileName: string, qualityList: LX.QualityList, listId?: string) => {
  ...
  const downloadInfo: LX.Download.ListItem = {
    ...
    metadata: {
      ...
      filePath: '',
      listId,
      fileName: replaceInvalidFileNameChars(`${clipFileNameLength(formatMusicName(fileName, musicInfo.name, clipNameLength(musicInfo.singer)))}.${ext}`),
    },
  }
